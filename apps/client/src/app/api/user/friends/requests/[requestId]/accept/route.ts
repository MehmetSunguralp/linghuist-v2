...
export async function PUT(request: Request, context: { params: Promise<{ requestId: string }> }) {
  const base = apiBase();
  if (!base) {
  ...
  const res = await fetch(`${base}/api/user/friends/requests/${encodeURIComponent(requestId)}/accept`, {
    method: 'PUT',
    headers: {
